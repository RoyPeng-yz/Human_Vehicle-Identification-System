...
from Detection.keras_retinanet.models.resnet import resnet50_retinanet
from Detection.keras_retinanet.utils.image import read_image_bgr, read_image_array, read_image_stream, \
    preprocess_image, resize_image
from Detection.keras_retinanet.utils.visualization import draw_box, draw_caption
from Detection.keras_retinanet.utils.colors import label_color
        ...
        if (self.__modelLoaded == False):
            ...
            try:
                if (self.__modelType == "retinanet"):
                    ...
                    detected_copy = cv2.cvtColor(detected_copy, cv2.COLOR_BGR2RGB)
                    print("车辆数量是：", vehicle_num)
                    print("人数是：", person_num)
                    # 显示检测结果
                    show = detected_copy.copy()
                    cv2.rectangle(show, (show.shape[1] - 100, 0), (show.shape[1], 40), (255, 255, 255), -1)
                    cv2.putText(show, 'vehicle_num: ' + str(vehicle_num), (show.shape[1] - 95, 12),
                                cv2.FONT_HERSHEY_COMPLEX, 0.3, (0, 0, 0), 1)
                    cv2.putText(show, 'person_num: ' + str(person_num), (show.shape[1] - 95, 32),
                                cv2.FONT_HERSHEY_COMPLEX, 0.3, (0, 0, 0), 1)
                    cv2.imshow('out', show)
                    cv2.waitKey(0)
                    detected_copy = cv2.cvtColor(detected_copy, cv2.COLOR_BGR2RGB)
